...
export const initCronJobs = () => {
  ...
  cron.schedule('0 7 * * *', async () => {
    ...
    for (const user of users) {
      ...
      if (entries.length > 0) {
        ...
        logger.info('Cron: On This Day cached', { userId: user.id, count: entries.length });
      }
    }
  }, { timezone: 'Asia/Jakarta' });
  ...
  // ─── Weekly: Cleanup expired refresh tokens (Minggu 02:00) ──
  cron.schedule('0 2 * * 0', () => {
    ...
    `).run();
    logger.info('Cron: Cleanup done', { deleted: result.changes });
  }, { timezone: 'Asia/Jakarta' });

  // ─── Monthly: Pre-generate Year in Review (1 Jan 00:30) ──
  cron.schedule('30 0 1 1 *', async () => {
    ...
    for (const user of users) {
      ...
      `).all(user.id, String(lastYear)) as any[];
